...
// TheGates: optional renderer-specific addend, set by the host module before
// calling mozilla::StartMacSandbox. Defined in
// modules/the_gates/sandbox/macos/sandbox_macos.mm.
extern "C" const char *tg_renderer_sandbox_addend;
...
void MacSandboxInfo::AppendAsParams(std::vector<std::string>& aParams) const {
  ...
  // TheGates: only Content is supported on this fork; other Firefox process
  // types were stripped out during vendoring (see firefox-shim/mac/README.md).
  MOZ_RELEASE_ASSERT(this->type == MacSandboxType_Content);
...
bool StartMacSandbox(MacSandboxInfo const& aInfo, std::string& aErrorMessage) {
  ...
  if (aInfo.type == MacSandboxType_Content) {
    ...
    if (aInfo.level >= 1) {
      ...
      // TheGates: append renderer-specific addend if the host module set it.
      // See modules/the_gates/sandbox/macos/sandbox_macos.mm.
      if (::tg_renderer_sandbox_addend != nullptr) {
        profile.append(::tg_renderer_sandbox_addend);
      }
    } else {
      fprintf(stderr,
...
bool StartMacSandboxIfEnabled(const MacSandboxType aSandboxType, int aArgc,
  ...
  // TheGates: only Content is supported on this fork; other Firefox process
  // types were stripped out during vendoring (see firefox-shim/mac/README.md).
  MOZ_RELEASE_ASSERT(aSandboxType == MacSandboxType_Content);
