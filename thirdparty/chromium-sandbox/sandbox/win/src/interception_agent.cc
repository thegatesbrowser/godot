...
InterceptionAgent* InterceptionAgent::GetInterceptionAgent() {
  ...
  if (!s_singleton) {
    ...
    wprintf(L"InterceptionAgent::GetInterceptionAgent success: %d\n", success);
    if (!success) {
...
bool InterceptionAgent::Init(SharedMemory* shared_memory) {
  ...
  wprintf(L"InterceptionAgent::Init num_intercepted_dlls: %d\n", shared_memory->num_intercepted_dlls);
...
bool InterceptionAgent::OnDllLoad(const UNICODE_STRING* full_path,
  ...
  wprintf(L"InterceptionAgent::OnDllLoad PatchDll dll_name: %s\n", dll_info->dll_name);
...
bool InterceptionAgent::PatchDll(const DllPatchInfo* dll_info,
  ...
  for (size_t i = 0; i < dll_info->num_functions; i++) {
    ...
    wprintf(L"InterceptionAgent::PatchDll Setup function: %s, interceptor: %s\n", function->function, interceptor);
