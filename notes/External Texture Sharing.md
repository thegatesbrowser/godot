...
- **The handshake is order-sensitive.** Renderer sends `send_filehandle` first, then blocks on `recv_filehandle`. The launcher's `RenderResult.create_external_texture()` must have completed the VMA allocation before responding, or it tells the renderer there's no handle yet. Today the launcher creates the texture synchronously on the `gate_entered` signal — fast enough that the renderer's first `send_filehandle` retry succeeds.
- **External memory MUST use dedicated allocation.** Any `vmaCreateImage` that exports via FD/HANDLE/IOSurface needs `VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT` set in the alloc flags. Otherwise VMA sub-allocates multiple `VkImage`s into one `VkDeviceMemory` block; the exported FD references the whole block; and the importing process sees neighboring textures (launcher UI textures, etc.) through the shared handle. This was the cause of the visible pixel-corruption screenshots in the 2026-05-23 session — see [[Sandboxing/Bootup Performance#The external texture aliasing trap]] for the full investigation.
